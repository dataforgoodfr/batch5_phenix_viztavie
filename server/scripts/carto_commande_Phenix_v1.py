...
fname_P  = './Phenix_DB_clean/' + lst_table[6] + '.csv'
fname_EC = './Phenix_DB_clean/EC' + lst_table[3] + '.csv'
fname_RC = './Phenix_DB_clean/RC' + lst_table[3] + '.csv'
df_P_diff, df_EC_diff ,df_RC_diff = phnx.create_diff_df (df_P, df_EC, df_RC, fname_P, fname_EC, fname_RC)

if df_P_diff.size != 0:
    # Add food group to Product table
    df_P_diff = phnx.add_foodgroup(df_P_diff,csv_path='./OFF_DB/mehdi_phenix_foodgroup.csv')

    # Estimate quantity with using 4 methods
    df_P1 = phnx.add_product_qty(df_P_diff)
    df_P2 = phnx.get_qty_from_PhenixCol(df_P_diff)
    df_P3 = phnx.extrapolate_qty_from_productvalue(df_P_diff,df_OP,df_CP)
    OFF_csv = './OFF_DB/fr.openfoodfacts.org.products.csv'
    df_P4 = phnx.get_qty_from_OFF(df_P_diff,OFF_csv=OFF_csv)

    df_P1.set_index(['P_Id'],drop=False,inplace=True)
    df_P2.set_index(['P_Id'],drop=False,inplace=True)
    df_P3.set_index(['P_Id'],drop=False,inplace=True)
    df_P4.set_index(['P_Id'],drop=False,inplace=True)
    Qty_val = phnx.prep_qty_val(df_P1, df_P2, df_P3, df_P4)

    # Select the relevant method for quantity estimation
    Qty_val = phnx.select_qty_method(Qty_val)

    df_P_diff.set_index('P_Id',drop=False,inplace=True)
    df_P_diff['Qty_val'] = Qty_val.value
    df_P_diff.reset_index(drop=True,inplace=True)

# Correct GPS coordinates in Comptes
#df_geo_matrix = phnx.geotag_Compte(df_EC_diff,prefix='EC_',IGN_API='YOUR_API_KEY')
...
df_EC_diff = phnx.replace_geotag(df_EC_diff,df_geo_matrix,prefix='EC_', threshold=1)
df_RC_diff = phnx.replace_geotag(df_RC_diff,df_geo_matrix,prefix='RC_', threshold=1)

# Merge df_diff (Comptes and Produits) into cleaned df
df_P_cleaned, df_EC_cleaned, df_RC_cleaned = phnx.get_cleaned_df(df_P, df_EC, df_RC, fname_P, fname_EC, fname_RC) 

df_P_new = pd.concat([df_P_cleaned, df_P_diff],axis=0, ignore_index=True)
df_EC_new = pd.concat([df_EC_cleaned, df_EC_diff],axis=0, ignore_index=True)
df_RC_new = pd.concat([df_RC_cleaned, df_RC_diff],axis=0, ignore_index=True)
...
df_EC_new.to_csv  (phenix_clean_path + 'EC' + lst_table[3] + '.csv',sep='\t',encoding='utf-8')
df_RC_new.to_csv  (phenix_clean_path + 'RC' + lst_table[3] + '.csv',sep='\t',encoding='utf-8')
...
df_P_new.to_csv   (phenix_clean_path + lst_table[6] + '.csv',sep='\t',encoding='utf-8') 
...
commands = df_Phenix[["CO_DateCommande","EC_Id","RC_Id","CP_QuantiteTotale","CP_QuantiteUnite","P_food_group","Qty_val"]].copy()
...
start = time.clock()
commands['Qty_totale'] = np.where(np.isin(commands['CP_QuantiteUnite'],['Kg','kg','Kilo','Litre','Litres']), commands['CP_QuantiteTotale'] * 1000, np.where(np.isin(commands['CP_QuantiteUnite'],['Unités','Unité','unité','unités','pièces','Pcs']),commands['Qty_val'] * commands['CP_QuantiteTotale'],float('nan')))
end = time.clock()
print('processing time: ',round(end - start),' seconds')
...
print('----> to list ---->')
start = time.clock()
commands_id.reset_index(level=["CO_DateCommande","EC_Id","RC_Id"], inplace=True)
